...
! Generated by on {}""".format(datetime.datetime.now())
    ...
    inp_data += gen_inp.run_name(run_name)
    inp_data += gen_inp.sim_type(moves.ensemble)

    # Number of species
    inp_data += gen_inp.nbr_species(nbr_species)
    ...
    inp_data += gen_inp.vdw_style(vdw_styles,cutoff_styles,vdw_cutoffs)
    ...
    inp_data += gen_inp.charge_style(charge_styles,charge_cutoffs,
    ...
    inp_data += gen_inp.mixing_rule(mixing_rule)
    ...
    if 'seeds' in kwargs:
        ...
        seed1 = None
        seed2 = None
    inp_data += gen_inp.seed_info(seed1,seed2)
    ...
    if 'rcut_min' in kwargs:
        ...
        rcut_min = 1.0
    inp_data += gen_inp.minimum_cutoff(rcut_min)
    ...
    if 'pair_energy' not kwargs:
        ...
        pair_energy=True
    inp_data += gen_inp.pair_energy(pair_energy)
    ...
    inp_data += gen_inp.molecule_files(max_molecule_dict)
    ...
    inp_data += gen_inp.temperature_info(temperatures)
    ...
    if moves.ensemble == 'npt' or moves.ensemble == 'gemc_npt':
        ...
        inp_data += gen_inp.pressure_info(pressures)
    ...
    if moves.ensemble == 'gcmc':
        ...
        inp_data += gen_inp.chemical_potential_info(chemical_potentials)
    ...
    inp_data += gen_inp.move_probability_info(move_prob_dict)
    ...
    inp_data += gen_inp.start_type(start_types)
    ...
    inp_data += gen_inp.run_type(run_type,thermal_stat_freq,vol_stat_freq)
    ...
    inp_data += gen_inp.simulation_length_info(units,prop_freq,
    ...
    if 'properties' in kwargs:
        ...
        properties = ['energy_total', 'energy_lj', 'energy_elec',
                      'energy_intra', 'enthalpy', 'pressure',
                      'volume', 'nmols', 'mass_density']

    inp_data += gen_inp.properties(properties,nbr_boxes)

    # Empty fragment section
    inp_data += gen_inp.fragment_files()
    ...
    if verbose_log:
        inp_data += gen_inp.verbose_log(verbose_log)
    ...
    inp_data += gen_inp.cbmc_info(cbmc_kappa_ins, cbmc_kappa_dih,
            cbmc_rcuts)
...
def property_info(properties,nbr_boxes):
    ...
    if not isinstance(properties,list):
        raise TypeError('Properties should be specified '
                'as a list of properties to be printed')
    
    valid_properties = ['energy_total', 'energy_lj', 'energy_elec',
                      'energy_intra', 'enthalpy', 'pressure',
                      'volume', 'nmols', 'density', 'mass_density']

    for prop in properties:
        if prop not in valid_properties:
            raise ValueError('Invalid property: {}. Valid choices '
...
def cbmc_info(n_insert, n_dihed, cutoffs):
