...
""".format(datetime.datetime.now())
    ...
    if ( moves.ensemble == 'npt' or
         moves.ensemble == 'gemc' or
         moves.ensemble == 'gemc_npt' ):
    ...
    inp_data += get_simulation_length_info(units,prop_freq,
    ...
    inp_data += get_cbmc_info(cbmc_kappa_ins, cbmc_kappa_dih,
            cbmc_rcuts)
...
def get_charge_style(charge_styles, cutoffs,
...
{nbr_boxes}""".format(nbr_boxes=nbr_boxes)
    ...
    for box,box_type in zip(boxes,box_types):
        inp_data += """
{box_type}""".format(box_type=box_type)
        if box_type == 'cubic':
            inp_data += """
{dim}
...
"""
    ...
    if 'angle' in kwargs:
        ...
        inp_data += """
# Prob_Angle
{prob_angle}""".format(prob_angle=angle)
        inp_data += """
!------------------------------------------------------------------------------
"""
...
def get_simulation_length_info(units,prop_freq,coord_freq,length,
